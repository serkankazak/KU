...
function l() {
	chrome.storage.local.get(["id", "pass"], function(d) {
		...
		if (($("button").html() == "Go") && (d["id"] ? (d["id"].length > 0) : false) && (d["pass"] ? (d["pass"].length > 0) : false)) {
			$.ajax({
				...
				success: function(resp) {
					if (!resp.toString().includes("My academic level")) {
						...
						$("body").append('<div align="right"><div align="left" style="width:100px; background-color:#DDDDDD"><div id="prog" style="background-color:#4CAF50; height:10px; width: 0%"></div></div></div>');
						ipr += 10; $("#prog").css("width", ipr + "%");
